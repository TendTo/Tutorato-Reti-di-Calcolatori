...
class BitArray {
    constructor(value) {
        if (typeof value === "number") {
            ...
            throw new TypeError("Invalid type. Must be number, BitArray, Uint32Array or Uint8Array");
        }
    }
    get length() {
        return this._length;
     ...
     */
    set(idx, value) {
        assert(idx >= 0 && idx < this.length, `Index must be between 0 and ${this.length - 1}`, RangeError);
    ...
    equals(x) {
        if (this.length !== x.length)
            return false;
        for (let i = 0; i < this._wordArray.length; i++) {
            if (this._wordArray[i] !== x._wordArray[i])
                return false;
        }
        return true;
    }
...
class IP {
    ...
    constructor(value, mask) {
        ...
        if (!this.isValidMask(mask))
            throw new TypeError(`Mask must be a number between 0 and ${this.maxMask}`);
        this._mask = mask;
    }
    set mask(value) {
        assert(this.isValidMask(value), `Mask must be a number between 0 and ${this.maxMask}`, RangeError);
        this._mask = value;
    }
    get numIPs() {
        return Math.pow(2, (this.maxMask - this._mask));
    }
}
class IPv4 extends IP {
    constructor(value, mask) {
        if (value instanceof Uint8Array) {
            ...
            value = _a._parseArray(value);
        }
        else if (typeof value === "string") {
            value = _a._parseString(value);
        }
        super(value, mask);
    }
    static _parseString(value) {
        const match = value.match(_a.ipRegex);
        if (!match || match.length !== 5)
            throw new TypeError(`Invalid IPv4 address: '${value}'`);
        return new Uint8Array(match.slice(1).map((octet) => {
    ...
    static _parseArray(value) {
        ...
        assert(value.every((octet) => octet >= 0 && octet <= 255), "Elements of the array must be between 0 and 255", RangeError);
        return new Uint8Array(value);
    }
    get network() {
        const networkBits = this._bits.copy();
        networkBits.setToFalseAfter(this._mask);
        return new _a(networkBits, this._mask);
    }
    get broadcast() {
        const broadcastBits = this._bits.copy();
        broadcastBits.setToTrueAfter(this._mask);
        return new _a(broadcastBits, this._mask);
    }
    get numHosts() {
        if (this._mask === this.maxMask)
            return 1;
        if (this._mask === this.maxMask - 1)
            return 2;
        return Math.pow(2, (this.maxMask - this._mask)) - 2;
    }
    get left() {
        assert(this._mask < this.maxMask, "Cannot get children if mask is already 32", RangeError);
        const leftBits = this._bits.copy();
        leftBits.set(this._mask, false);
        return new _a(leftBits, this._mask + 1);
    }
    get right() {
        assert(this._mask < this.maxMask, "Cannot get children if mask is already 32", RangeError);
        const rightBits = this._bits.copy();
        rightBits.set(this._mask, true);
        return new _a(rightBits, this._mask + 1);
    }
    getChildren() {
        assert(this._mask < this.maxMask, "Cannot get children if mask is already 32", RangeError);
        ...
        return [
            new _a(left, this._mask + 1),
            new _a(right, this._mask + 1),
        ];
    }
    toString() {
...
_a = IPv4;
IPv4.ipv4PartRegex = "(0*(?:25[0-5]|(?:2[0-4]|1\\d|[1-9]|)\\d))";
IPv4.ipRegex = new RegExp(`^${_a.ipv4PartRegex}\\.${_a.ipv4PartRegex}\\.${_a.ipv4PartRegex}\\.${_a.ipv4PartRegex}$`);
window.onload = () => {
