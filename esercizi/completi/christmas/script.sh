...
vms=("CClient1" "CClient2" "CServer" "CRouter")
...
function setup() {
    ...
    vboxmanage modifyvm "${vms[1]}" --memory 512 --nic1 intnet --intnet1 lan1
    vboxmanage modifyvm "${vms[2]}" --memory 512 --nic1 intnet --intnet1 lan2
    vboxmanage modifyvm "${vms[3]}" --memory 512 --nic1 intnet --intnet1 lan1 --nic2 intnet --intnet2 lan2
    ...
    vboxmanage guestcontrol "${vms[0]}" run --exe /sbin/ip $credentials --wait-stdout -- addr add 165.64.0.1/21 dev enp0s3
    vboxmanage guestcontrol "${vms[0]}" run --exe /sbin/ip $credentials --wait-stdout -- route add default via 165.64.7.254

    echo "Configurazione nodo ${vms[1]}"
    vboxmanage guestcontrol "${vms[1]}" run --exe /sbin/ip $credentials --wait-stdout -- addr add 165.64.0.2/21 dev enp0s3
    vboxmanage guestcontrol "${vms[1]}" run --exe /sbin/ip $credentials --wait-stdout -- route add default via 165.64.7.254

    echo "Configurazione nodo ${vms[2]}"
    vboxmanage guestcontrol "${vms[2]}" run --exe /sbin/ip $credentials --wait-stdout -- addr add 165.64.8.1/23 dev enp0s3
    vboxmanage guestcontrol "${vms[2]}" run --exe /sbin/ip $credentials --wait-stdout -- route add default via 165.64.9.254

    echo "Configurazione router ${vms[3]}"
    vboxmanage guestcontrol "${vms[3]}" run --exe /sbin/ip $credentials --wait-stdout -- link set enp0s8 up
    vboxmanage guestcontrol "${vms[3]}" run --exe /sbin/ip $credentials --wait-stdout -- addr add 165.64.7.254/21 dev enp0s3
    vboxmanage guestcontrol "${vms[3]}" run --exe /sbin/ip $credentials --wait-stdout -- addr add 165.64.9.254/23 dev enp0s8
    vboxmanage guestcontrol "${vms[3]}" run --exe /usr/sbin/sysctl $credentials  --wait-stdout -- -w net.ipv4.ip_forward=1
