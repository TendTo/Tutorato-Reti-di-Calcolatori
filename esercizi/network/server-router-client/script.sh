...
local_path="/home/tend/Programming/Tutorato/Tutorato-Reti-di-Calcolatori/esercizi/socket/reverse-shell"
vms=("Server" "Router" "Client")
...
function setup() {
    ...
    vboxmanage modifyvm "${vms[1]}" --memory 512 --nic1 intnet --intnet1 lan1 --nic2 intnet --intnet2 lan2
    ...
    vboxmanage guestcontrol "${vms[0]}" run --exe /sbin/ip $credentials --wait-stdout -- ip addr add 10.0.1.1/24 dev enp0s3
    vboxmanage guestcontrol "${vms[0]}" run --exe /sbin/ip $credentials --wait-stdout -- ip route add default via 10.0.1.254

    echo "Configurazione router ${vms[1]}"
    vboxmanage guestcontrol "${vms[1]}" run --exe /sbin/ip $credentials --wait-stdout -- ip link set enp0s8 up
    vboxmanage guestcontrol "${vms[1]}" run --exe /sbin/ip $credentials --wait-stdout -- ip addr add 10.0.1.254/24 dev enp0s3
    vboxmanage guestcontrol "${vms[1]}" run --exe /sbin/ip $credentials --wait-stdout -- ip addr add 192.168.1.254/24 dev enp0s8
    vboxmanage guestcontrol "${vms[1]}" run --exe /usr/sbin/sysctl $credentials  --wait-stdout -- sysctl -w net.ipv4.ip_forward=1

    echo "Configurazione nodo ${vms[2]}"
    vboxmanage guestcontrol "${vms[2]}" run --exe /sbin/ip $credentials --wait-stdout -- ip addr add 192.168.1.1/24 dev enp0s3
    vboxmanage guestcontrol "${vms[2]}" run --exe /sbin/ip $credentials --wait-stdout -- ip route add default via 192.168.1.254
